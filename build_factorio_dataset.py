    ...
    def build_planets(
        ...
        for section in ("planet", "surface"):
            for key, raw in sorted(self.raw.get(section, {}).items()):
                ...
                local_resources = sorted(key for key in entity_settings if key in resource_keys)
...
def main() -> None:
    ...
    parser.add_argument("--calculator-dir", type=Path, default=Path(__file__).resolve().parent)
    ...
    output = args.output or calculator_dir / "data" / DATASET_NAME
    report = args.report or calculator_dir / "data" / f"space-age-{TARGET_VERSION}-build-report.json"
    old_data = calculator_dir / "data" / "space-age-2.0.55.json"
    images_dir = calculator_dir / "images"
