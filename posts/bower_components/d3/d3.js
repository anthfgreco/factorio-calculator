!(function () {
  ...
  var d3_formatPrefixes = ["y", "z", "a", "f", "p", "n", "µ", "m", "", "k", "M", "G", "T", "P", "E", "Z", "Y"].map(
    d3_formatPrefix,
  )
  ...
  function d3_geo_projectionMutator(projectAt) {
    ...
    projection.clipAngle = function (_) {
      ...
      preclip =
        _ == null ? ((clipAngle = _), d3_geo_clipAntimeridian) : d3_geo_clipCircle((clipAngle = +_) * d3_radians)
      return invalidate()
    }
  ...
  function d3_scale_ordinal(domain, ranger) {
    ...
    scale.rangeRoundPoints = function (x, padding) {
      ...
      range = steps(
        start + Math.round((step * padding) / 2 + (stop - start - (domain.length - 1 + padding) * step) / 2),
        step,
      )
      rangeBand = 0
      ranger = {
  ...
  var d3_category20 = [
    2062260, 11454440, 16744206, 16759672, 2924588, 10018698, 14034728, 16750742, 9725885, 12955861, 9197131, 12885140,
    14907330, 16234194, 8355711, 13092807, 12369186, 14408589, 1556175, 10410725,
  ].map(d3_rgbString)
  var d3_category20b = [
  ...
  var d3_category20c = [
    3244733, 7057110, 10406625, 13032431, 15095053, 16616764, 16625259, 16634018, 3253076, 7652470, 10607003, 13101504,
    7695281, 10394312, 12369372, 14342891, 6513507, 9868950, 12434877, 14277081,
  ].map(d3_rgbString)
  ...
  function d3_scale_quantile(domain, range) {
    ...
    scale.invertExtent = function (y) {
      ...
      return y < 0
        ? [NaN, NaN]
        : [y > 0 ? thresholds[y - 1] : domain[0], y < thresholds.length ? thresholds[y] : domain[domain.length - 1]]
    }
    scale.copy = function () {
        ...
        if (da < π) {
          var oc =
            ...
            kc =
              1 /
              Math.sin(
                Math.acos((ax * bx + ay * by) / (Math.sqrt(ax * ax + ay * ay) * Math.sqrt(bx * bx + by * by))) / 2,
              ),
  ...
  d3_time_format.iso =
    Date.prototype.toISOString && +new Date("2000-01-01T00:00:00.000Z") ? d3_time_formatIsoNative : d3_time_formatIso
  ...
  function d3_time_scale(linear, methods, format) {
    ...
    scale.nice = function (interval, skip) {
      var domain = scale.domain(),
        ...
        method =
          interval == null ? tickMethod(extent, 10) : typeof interval === "number" && tickMethod(extent, interval)
