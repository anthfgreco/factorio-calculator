...
let DEFAULT_BUILDINGS = new Set(["assembling-machine-1", "electric-furnace", "electric-mining-drill"])
...
function getBuildingGroups(buildings, recipes) {
  ...
  let groups = new Map()
  for (let { categories, buildings } of sets as Set<any>) {
    if (buildings.size === 0) {
      continue
    }
...
class FactorySpecification {
  ...
  _syncPlanetDisable() {
    ...
    if (this.selectedPlanets.size === 0) {
      ...
      let planets: any[] = Array.from(this.selectedPlanets)
  ...
  isDefaultPlanet() {
    ...
    let a: any[] = Array.from(this.selectedPlanets)
