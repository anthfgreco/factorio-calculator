...
class PriorityLevel {
  ...
  constructor(list) {
    ...
    this.middle = null
    this.list = list
    this.div = d3.create("div").datum(this).classed("resource-tier", true)
    let self = this
...
export class PriorityList {
  ...
  static getDefaultArray(recipes) {
    let a: Map<any, any>[] = []
  ...
  renderEmpty() {
    ...
    let less = this.div.append("div").classed("resource-tier bookend", true)
    ...
    let more = this.div.append("div").classed("resource-tier bookend", true)
  ...
  _makeMiddle(level) {
    let self = this
    let middle = d3.create("div").datum(level).classed("middle", true)
